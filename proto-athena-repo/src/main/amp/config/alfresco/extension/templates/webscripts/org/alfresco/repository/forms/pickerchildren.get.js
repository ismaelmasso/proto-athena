...
function main()
{
   ...
   if (logger.isLoggingEnabled())
   {
      ...
      //RRD - Adding siteFilter
      logger.log("siteFilter = " + siteFilter);
   ...
   try
   {
      ...
      else if (url.templateArgs.type == "authority")
      {
         ...
         {
            findUsers(argsSearchTerm, maxResults, results, siteFilter);
         }
         else if (argsSelectableType == "cm:authorityContainer")
         {
            findGroups(argsSearchTerm, maxResults, results, siteFilter);
         }
         else
         {
...
function findUsers(filterTerm, maxResults, results, siteFilter)
{
   ...
   {
      //RRD - filter out unwanted results, only if siteFilter is true
      personResult = createPersonResult(search.findNode(personRef));
      if (userSites == [] || sitesMembership(personResult, userSites)) {
          // add to results
          results.push(
          {
             item: personResult,
...
function findGroups(searchTerm, maxResults, results, siteFilter)
{
   ...
   for each(var group in searchResults)
   {
      ...
      //RRD - filter out unwanted results, only if siteFilter is true
      if (userSites == [] || sitesGroupMembership(group, userSites)) {
          // add to results
          results.push(
          {
...
//RRD - Return true if a group belongs (aka whitelisted) to at least one site in common with the user currently logged in
function sitesGroupMembership(groupResult, currentUserSites) {
  var foundCommonSite = false;
  currentUserSites.filter(function(n) {
    var groupNames = n.node.properties["rrdathena:groupNames"];
    if (arrContains(groupNames,groupResult.fullName)) {
      foundCommonSite = true;
    }
  });
  return foundCommonSite;
